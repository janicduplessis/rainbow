...
import { DAPP_BROWSER, LAZY_TABS, RNBW_MEMBERSHIP, RNBW_REWARDS, useExperimentalFlag } from '@/config';
    ...
    () =>
      stateRef.current.routes.map((route: RouteProp<ParamListBase, string>, index: number) => {
        if (
          ...
          (!showRnbwRewardsTab && (route.name === Routes.RNBW_REWARDS_SCREEN || route.name === Routes.RNBW_MEMBERSHIP_SCREEN))
        ) {
          return null;
        }
